...
class madhat_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'madhat_rp', // Base ID
			__('MadHat Recent Posts with Thumbnails', 'madhat'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'madhat' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-date'><?php echo the_time(__('M j, Y','madhat')); ?></div>
		         </li>      
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','madhat'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'madhat' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'madhat' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','madhat' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'madhat' ); ?></label>
...
add_action( 'widgets_init', 'register_madhat_widget' );  
function register_madhat_widget() {  
    register_widget( 'madhat_Recent_Posts' );  
}  
